    ...
    <main className="flex-1">
      ...
      <section className="mx-auto max-w-4xl px-6 py-6">
        <div className="flex flex-wrap items-center gap-x-6 gap-y-2 border-y py-4 font-mono text-sm text-muted-foreground">
          <span className="text-xs uppercase tracking-wider">{s.runsOn}</span>
     ...
     ─▶ placeholders ─▶ LLM
      ...
      <section className="mx-auto max-w-4xl px-6 py-12">
        <Card className="flex flex-col items-start gap-4 p-8 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="font-mono text-xl font-semibold tracking-tight">{s.ctaTitle}</h2>
      ...
      <footer className="mx-auto max-w-4xl px-6 py-10 text-sm text-muted-foreground">
        <div className="flex flex-wrap items-center justify-between gap-4 border-t pt-6">
          ...
          <div className="flex gap-4">
            <Link href={`/${lang}/docs`} className="hover:text-foreground">Docs</Link>
            <Link href={`/${lang}/docs/playground`} className="hover:text-foreground">Playground</Link>
