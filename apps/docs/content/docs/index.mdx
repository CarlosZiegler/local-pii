...
- **Expo / React Native** → [`rampart()`](/en/docs/expo) via `onnxruntime-react-native`.
- **Browser / web** → [`rampartWeb()`](/en/docs/browser) via `onnxruntime-web` (WASM/WebGPU).
...
- [Vercel AI SDK](/en/docs/adapters) — `withPii(model)`
- [OpenAI & Grok/xAI](/en/docs/adapters) — `withPiiOpenAI(client)`

## Where next

- [Core Concepts](/en/docs/concepts) — the pipeline, sessions, the vault, trust boundaries
- [Placeholder Strategies](/en/docs/strategies) — `sequential`, `hashed`, `token`
- [Tool Calls](/en/docs/tool-calls) — the hard part, solved
- [Limitations](/en/docs/limitations) — what it does **not** catch (read this)
